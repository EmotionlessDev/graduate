...
\begin{itemize}
    ...
    \item \textbf{Интеграция с Windows API}: библиотека \texttt{pywin32} предоставляет удобный доступ к функциям захвата окон, необходимым для работы с покер-клиентом.
\end{itemize}
...
Система построена по модульному принципу с чётким разделением ответственности. Общая структура представлена на рисунке~\ref{fig:component_diagram}.
...
\begin{figure}[h]
    ...
    \fbox{
        \parbox{0.9\textwidth}{
            \centering
            \textbf{Диаграмма компонентов системы} \\
            \vspace{2cm}
            \vspace{1cm}
            Компоненты: \\
            \texttt{service.py} (точка входа) \\
            $\downarrow$ \\
            \texttt{PokerStateManager} (оркестратор) \\
            $\downarrow$ \\
            \texttt{BaseRoom / ReplayPokerRoom} (логика рума) \\
            $\downarrow$ \\
            \begin{tabular}{ccc}
                \texttt{Vision Pipeline} & \texttt{Extractors} & \texttt{Advisor} \\
                (геометрия) & (OCR, NN) & (рекомендации)
            \end{tabular}
            \vspace{2cm}
        }
    }
...
% TODO: Добавить скрины интерфейса с реального покер-клиента;
